...
export async function createExamen(data: examen) {
    try {
        if (!data.id_asign) {
            throw new Error("El id_asign es obligatorio");
        }

        const aulas = Array.isArray(data.aulaAlumnos) ? data.aulaAlumnos : [];
...
export async function updateExamen(id: string, body: any) {
    try {
        const allowedFields = [
            ...
            "n_aprobados", "finalizado", "partes", "aulaAlumnos"
        ];
